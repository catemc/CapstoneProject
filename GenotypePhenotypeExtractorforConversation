...
def load_conversation_context(path: str) -> str:
    """
    Load prior conversation / evaluation history as read-only context.
    """
    ...
    def extract(self):
        if len(self.conversation) == 0:
            ...
            if self.conversation_context:
                ...
                The following information contains prior extraction attempts, evaluation
                results, and identified errors. Use this context ONLY to avoid repeating
                mistakes. Do NOT infer new mutation–effect pairs unless explicitly supported
                by the paper text below.
